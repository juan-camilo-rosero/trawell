  ...
  useEffect(() => {
    if (!mockLoading && mockItinerary) {
      ...
      mockItinerary.days.forEach((day) => {
        ...
        console.log("─────────────────────────────────────");
        console.log(`Total de actividades: ${day.items.length}\n`);
        ...
        day.items.forEach((item, index) => {
          ...
          console.log(`${index + 1}. ${icons[item.type]} ${item.title}`);
          ...
          if (item.flightDetails) {
            console.log(
              `   └─ Vuelo: ${item.flightDetails.carrierName} ${item.flightDetails.flightNumber}`
            );
            console.log(
              `      ${item.flightDetails.departureAirport} (${item.flightDetails.departureTime}) → ${item.flightDetails.arrivalAirport} (${item.flightDetails.arrivalTime})`
            );
            console.log(
              `      Duración: ${item.flightDetails.duration} | Escalas: ${item.flightDetails.numberOfStops}`
            );
          }

          if (item.accommodationDetails) {
            console.log(`   └─ Hotel: ${item.accommodationDetails.hotelName}`);
            console.log(
              `      Habitación: ${item.accommodationDetails.roomType}`
            );
            console.log(`      Noches: ${item.accommodationDetails.nights}`);
          }

          if (item.foodDetails) {
            console.log(
              `   └─ Restaurante: ${item.foodDetails.restaurantName}`
            );
            console.log(`      Cocina: ${item.foodDetails.cuisine}`);
            console.log(`      Tipo de comida: ${item.foodDetails.mealType}`);
            console.log(
              `      Rating: ${item.foodDetails.rating} ⭐ (${item.foodDetails.userRatingsTotal} reseñas)`
            );
            console.log(
              `      Nivel de precio: ${"$".repeat(
                item.foodDetails.priceLevel
              )}`
            );
          }

          if (item.touristSiteDetails) {
            console.log(`   └─ Sitio: ${item.touristSiteDetails.siteName}`);
            console.log(`      Categoría: ${item.touristSiteDetails.category}`);
            console.log(
              `      Duración estimada: ${item.touristSiteDetails.estimatedDuration}`
            );
            console.log(
              `      Entrada: ${
            ...
            if (item.touristSiteDetails.rating) {
              console.log(
                `      Rating: ${item.touristSiteDetails.rating} ⭐ (${item.touristSiteDetails.userRatingsTotal} reseñas)`
              );
            }
          }
